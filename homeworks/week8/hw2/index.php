<?
session_start();  // 1.可以從cookie拿到PHPSESSID
$is_login = false;
$username = '';
$nickname = '';
...
subeditBtn.forEach( btn => {
    btn.addEventListener("click", function() {
        // console.log(this.parentNode.parentNode.querySelector("#sub_comment_id").value)
        let comment_id = parseInt(this.parentNode.parentNode.querySelector("#sub_comment_id").value);
        let listItem = this.parentNode.parentNode;
        let editInput = this.parentNode.parentNode.querySelector(".editComment");
        let label = listItem.querySelector("label");
        let containsClass = listItem.classList.contains('editMode');
